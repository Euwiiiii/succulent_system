 ...
 */
export const calculateFinalPrices = ({
    plants = [],
    pot = null,
    potCost = 0,
    supplies = [],
    ...
    const totalSuppliesCost = supplies.reduce((sum, item) => {
        // Handle nested supply object when populated, or raw unitCost if passed directly from frontend state
        let unitCost = 0;
        if (item.supply && typeof item.supply === 'object') {
            unitCost = Number(item.supply.unitCost) || Number(item.supply.costPerGram) || 0;
        } else if (item.unitCost !== undefined) {
 ...
 */
export const formatCurrency = (amount, isSellingPrice = false) => {
