...
const CostingCalculator = () => {
    ...
    const handleSaveToInventory = async () => {
        ...
        try {
            const payload = {
                ...details,
                sellingPrice: recommendedPrice, // Ensure price is sent
                costPrice: totalCost,
    ...
    return (
        <div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                <h2 style={{ fontSize: '2ms', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', color: '#2d6a4f', fontWeight: 'bold' }}>
                    CREATE ARRANGEMENT & COSTING 
                </h2>
                <p style={{ color: 'gray', fontSize: '1.2ms' }}>
            ...
            <div style={{ display: 'flex', gap: '40px', justifyContent: 'center', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, maxWidth: '500px' }}>
                    ...
                    <div style={{ marginBottom: '25px', padding: '20px', backgroundColor: '#f8fffb', borderRadius: '10px', border: '1px solid #d4edda', textAlign: 'center' }}>
                        <h4 style={{ color: '#2d6a4f', marginBottom: '15px' }}>🪴 Plants Included</h4>
                        {plants.map((plant) => (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            ...
                            <input name="imageUrl" type="text" value={supplies.imageUrl} onChange={handleSupplyChange} style={{...inputStyle, width: '95%'}} />
                        </div>
                    </div>
                </div>
                ...
                <div style={{ width: '350px', padding: '30px', backgroundColor: '#2d6a4f', color: 'white', borderRadius: '15px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }}>
                    <h3 style={{ textAlign: 'center', marginBottom: '25px', fontSize: '1.5ms' }}>Pricing Breakdown</h3>
                    ...
                    <div style={{ ...resultRow, margin: '20px 0', fontSize: '1.4ms', fontWeight: 'bold' }}><span>Base Cost:</span><span>₱{totalCost.toFixed(2)}</span></div>
                    <div style={resultRow}><span>Profit Margin:</span><span style={{ color: '#95d5b2' }}>+ ₱{profitAmount.toFixed(2)}</span></div>
                    <hr style={{ opacity: 0.3, margin: '20px 0' }} />
                    <div style={{ ...resultRow, fontSize: '1.8ms', fontWeight: 'bold' }}><span>Selling Price:</span><span>₱{recommendedPrice.toFixed(2)}</span></div>
...
const saveButtonStyle = { 
    ...
    color: '#1b4332', border: 'none', borderRadius: '8px', fontSize: '1.2ms', 
