...
const Sales = () => {
    ...
    return (
        <div style={{ padding: '40px', maxWidth: '900px', margin: '0 auto' }}>
            <h2 className="panel-header">Sales Tracker</h2>
            
            <div style={{ backgroundColor: '#f8fffb', padding: '20px', borderRadius: '15px', border: '1px solid var(--dark-green)', marginBottom: '30px', textAlign: 'center' }}>
                <h3 style={{ color: '#1b4332', margin: '0 0 10px 0' }}>Total Revenue (All Time)</h3>
                <h1 style={{ color: 'var(--dark-green)', margin: 0, fontSize: '3rem' }}>{formatCurrency(totalRevenueAllTime, true)}</h1>
            </div>
                    ...
                    {sales.length === 0 ? (
                        ...
                        sales.map(s => (
                            <tr key={s._id} style={{ borderBottom: '1px solid #ddd' }}>
                                ...
                                <td style={tdStyle}><span style={{ color: 'var(--dark-green)', fontWeight: 'bold' }}>{formatCurrency(s.totalRevenue, true)}</span></td>
                            </tr>
                        ))
                    )}
                </tbody>
